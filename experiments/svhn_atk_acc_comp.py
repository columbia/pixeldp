"""Run SVHN experiments for the paper.
"""
...
def run(plots_only=False):
    ...
    for model_name in ['pixeldp_resnet', 'robustop']:
        for attack_norm_bound in Ls:
            for noise_after_n_layers in [-1, 1]:
                # Add only one experiment for robustop
                if model_name == "robustop":
    ...
    pgd_sizes = [round(x, 2) for x in np.arange(0.1, 1.6, 0.1).tolist()]
        ...
        if hps.attack_norm_bound == 0.0 and model_name != 'robustop':
            ...
            if hps.attack_norm_bound not \
                ...
                if model_name == 'robustop':
                    ...
                    m = models.module_from_name(model_name)
                _models.append(m)
                _params.append(hps)
    ...
    dir_name = os.path.join(FLAGS.models_dir, 'svhn')
    ...
    xticks = [round(x, 2) for x in np.arange(0.1, 1.6, 0.1).tolist()]
    plots.plot_accuracy_under_attack.plot("svhn_accuracy_under_attack_robustop_comp",
                                          _models,
                                          _params,
                                          _attack_params,
                                          x_range=(0, 1.5),
