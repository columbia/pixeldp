...
class Model(object):
  ...
  def _build_model(self, inputs_tensor=None, labels_tensor=None):
      ...
      with tf.variable_scope('label_dup'):
        ...
        self.labels = tf.tile(self.labels, tf.concat([[self.hps.n_draws], ones], axis=0))

      assert self.mode == 'train' or self.mode == 'eval'
      logits, predictions, _ = self.inference(self.images, 1001)
      self.logits = logits
      self.predictions = predictions
      self.cost = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=logits, labels=tf.argmax(self.labels, 1)
        )
      )

  def inference(self, images, num_classes, for_training=False, restore_logits=True,
                scope=None, n_draws=1):
    ...
    return logits, predictions, auxiliary_logits
